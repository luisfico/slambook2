...
int main(int argc, char **argv)
{
  ...
  //Matching filter1: ORB
  find_feature_matches(img_1, img_2, keypoints_1, keypoints_2, matches);
  cout << "Matching filter1 ORB : Total correspondances" << matches.size() << endl;
  ...
  // Debug matching
  cv::Mat matchingImages = img_1.clone();
  matchingImages.push_back(img_2.clone());
  cv::Mat matchingImages2 = matchingImages.clone();
  ...
  cv::resize(matchingImages, matchingImagesShow, matchingImages.size() / 2);
  imshow("img matching", matchingImagesShow);
  waitKey(0); // debug



  //Matching filter2: Filter bad matches using fundamental matrix constraint
  vector<uchar> maskOutFundamentalmatrixFilter;
  findFundamentalMat(src, dst, FM_RANSAC, 3.0, 0.99, maskOutFundamentalmatrixFilter);

  for (size_t k = 0; k < maskOutFundamentalmatrixFilter.size(); k++)
  {
  ...
  int good_matches = sum(maskOutFundamentalmatrixFilter)[0];
  assert(good_matches >= 8);
  cout << "Matching filter2 Fudnamental : Total correspondances : " << good_matches << "/" << maskOutFundamentalmatrixFilter.size() << endl;

  cv::Mat matchingImagesShow2;
  cv::resize(matchingImages2, matchingImagesShow2, matchingImages2.size() / 2);
  imshow("img matching2", matchingImagesShow2);
  waitKey(0); // debug
  // waitKey(1);//release
